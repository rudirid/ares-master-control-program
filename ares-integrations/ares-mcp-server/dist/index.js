...
const TOOLS = [
    {
        ...
        description: "Retrieve Riord's proven coding patterns by tier (1=validated, 2=working, 3=experimental)",
        inputSchema: {
            type: "object",
            properties: {
                tier: {
    ...
    },
    {
        ...
        description: "Run Ares internal validation protocol on a proposed approach. Returns confidence score and recommended patterns.",
        inputSchema: {
            type: "object",
            properties: {
                task: {
    ...
    },
    {
        ...
        description: "Get the best proven pattern recommendation for a specific task",
        inputSchema: {
            type: "object",
            properties: {
                task: {
    ...
    },
    {
        ...
        description: "Get success rates and evidence for specific technologies from Riord's tech-success-matrix.md",
        inputSchema: {
            type: "object",
            properties: {
                technology: {
    ...
    try {
        switch (name) {
            ...
            case "validate_approach": {
                ...
                const approach = args?.proposed_approach;
                // Simple validation using pattern matching
                const lowerTask = (task + " " + approach).toLowerCase();
                const matchingPatterns = PATTERNS.filter((p) => p.appliesTo.some((keyword) => lowerTask.includes(keyword)));
...
`,
            ...
            case "recommend_pattern": {
                ...
                const matches = PATTERNS.filter((p) => p.appliesTo.some((keyword) => lowerTask.includes(keyword))).sort((a, b) => {
                    if (a.tier !== b.tier)
                        return a.tier - b.tier;
                    return b.successRate - a.successRate;
                });
                ...
                if (!data) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `No success data found for "${tech}".\n\nAvailable technologies: ${Object.keys(techData).join(", ")}`,
                            },
                        ],
                    };
                }
