...
class TimeFilter:
    ...
    def is_tradeable_by_time(
        ...
        if announcement_datetime.tzinfo is None:
            announcement_datetime = tz.localize(announcement_datetime)
        if current_datetime.tzinfo is None:
            current_datetime = tz.localize(current_datetime)
...
class TimeOfDayFilter:
    ...
    REALISTIC MODE: Market hours 7:00 AM - 4:30 PM AEST with time-weighted confidence
    - Pre-market (7-10 AM): Often most impactful announcements, premium weight
    - Prime time (10 AM-2 PM): Optimal liquidity, standard weight
    - Afternoon (2-4:30 PM): Acceptable liquidity, reduced weight
    - After hours: Low weight but still tradeable for major news
    """
    ...
    def __init__(
        ...
        self.market_open = time(hour=7, minute=0)  # REALISTIC: Include pre-market
        self.market_close = time(hour=16, minute=30)  # REALISTIC: Include afternoon
    ...
    def is_optimal_time(
        ...
        REALISTIC MODE: Use time-weighted confidence instead of hard blocks
        ...
        # REALISTIC: Pre-market (7-10 AM) - PREMIUM WEIGHT
        # Many major announcements drop before market open
        if self.market_open <= trade_time < self.optimal_start:
            return True, f"Pre-market ({trade_time.strftime('%H:%M')} AEST) - often most impactful", 0.10

        # Prime time (10 AM - 2 PM) - STANDARD WEIGHT
        elif self.optimal_start <= trade_time < self.optimal_end:
            return True, f"Prime time ({trade_time.strftime('%H:%M')} AEST) - optimal liquidity", 0.05

        # Afternoon (2 PM - 4:30 PM) - REDUCED WEIGHT but still tradeable
        elif self.optimal_end <= trade_time <= self.market_close:
            return True, f"Afternoon ({trade_time.strftime('%H:%M')} AEST) - acceptable liquidity", -0.05

        # After hours - LOW WEIGHT but don't block major announcements
        elif trade_time > self.market_close or trade_time < self.market_open:
            # Check if it's very early morning (1 AM - 7 AM) or evening (4:30 PM - 11 PM)
            if time(hour=1) <= trade_time < self.market_open:
                return True, f"Early morning ({trade_time.strftime('%H:%M')} AEST) - limited liquidity", -0.15
            else:
                return True, f"After hours ({trade_time.strftime('%H:%M')} AEST) - low liquidity", -0.20

        # Should never reach here, but safety fallback
        return True, f"Time: {trade_time.strftime('%H:%M')} AEST", -0.10
...
class MaterialityFilter:
    ...
    def __init__(self, min_materiality_score: float = 0.4):
        """
        Initialize materiality filter.

        REALISTIC MODE: Lower threshold to 0.4 (was 0.5)
        Many tradeable announcements score 0.4-0.6 range

        Args:
            min_materiality_score: Minimum score to trade (0-1, default 0.4)
        """
