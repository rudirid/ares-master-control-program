...
if __name__ == "__main__":
    ...
    print("\nTest 1: High confidence trade (0.75)")
    print("-"*80)
    shares, risk, details = sizer.calculate_position_size(
        symbol='BHP',
        entry_price=45.00,
        stop_loss_price=43.00,  # 2 points risk
        confidence=0.75
    )
    print(f"Position size: {shares} shares")
    print(f"Risk amount: ${risk:.2f}")
    print(f"Position value: ${details['position_value']:.2f}")
    print(f"Portfolio heat after: {details['portfolio_heat_after']:.1%}")

    print("\n\nTest 2: Open position and simulate win")
    print("-"*80)
    ...
    sizer.check_take_profit('BHP', 48.00)

    print("\n\nTest 3: Portfolio statistics")
    print("-"*80)
    stats = sizer.get_statistics()
    for key, value in stats.items():
        if isinstance(value, float):
            print(f"{key}: {value:.2f}" if 'pct' not in key.lower() and 'rate' not in key.lower() else f"{key}: {value:.1%}")
        else:
            print(f"{key}: {value}")

    print("\n" + "="*80)
    print("Kelly Criterion position sizer ready for integration!")
    print("="*80 + "\n")
