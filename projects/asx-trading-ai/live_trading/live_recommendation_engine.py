...
class LiveRecommendationEngine:
    ...
    def _initialize_components(self):
        ...
        # REALISTIC MODE: Lower materiality threshold from 0.5 to 0.4
        self.materiality_filter = MaterialityFilter(min_materiality_score=0.4)
    ...
    def extract_numeric_signals(self, title: str) -> Tuple[bool, str, float]:
        """
        Extract numeric signals from title (REALISTIC MODE).

        Numbers often matter more than sentiment in corporate announcements.

        Args:
            title: Announcement title

        Returns:
            Tuple of (has_signal, direction, confidence)
        """
        import re
        ...
        # Find dollar amounts
        dollar_pattern = r'\$(\d+(?:\.\d+)?)\s*([MmBb]?)'
        dollars = re.findall(dollar_pattern, title)

        # Positive keywords
        positive_keywords = ['up', 'increase', 'beat', 'exceed', 'above', 'higher', 'grew', 'growth',
                            'win', 'won', 'awarded', 'contract', 'agreement', 'acquisition']

        # Negative keywords
        negative_keywords = ['down', 'decrease', 'miss', 'below', 'lower', 'fell', 'declined',
                            'loss', 'cut', 'reduced']
        ...
        if percentages:
            for pct in percentages:
                value = float(pct)
                if value >= 5:  # Material change (5%+)
                    if any(pos in title_lower for pos in positive_keywords):
                        return True, 'POSITIVE', 0.7
                    elif any(neg in title_lower for neg in negative_keywords):
                        return True, 'NEGATIVE', 0.7
                    # If no context keyword, assume percentage alone is slightly positive
                    elif value >= 10:
                        return True, 'POSITIVE', 0.5

        # Check for large dollar amounts (contracts, acquisitions)
        if dollars:
            for amount, unit in dollars:
                value = float(amount)
                if unit.upper() == 'M' and value >= 10:  # $10M+
                    return True, 'POSITIVE', 0.6
                elif unit.upper() == 'B':  # Any billions
                    return True, 'POSITIVE', 0.8

        # Check for positive keywords even without numbers
        if any(kw in title_lower for kw in ['awarded', 'won', 'breakthrough', 'approval', 'major contract']):
            return True, 'POSITIVE', 0.5
    ...
    def process_announcement(self, announcement_id: int) -> Optional[Dict]:
        ...
        # 4. SENTIMENT ANALYSIS + NUMERIC SIGNALS (REALISTIC MODE)
        ...
        # REALISTIC MODE: Check for numeric signals
        has_numeric_signal, numeric_direction, numeric_confidence = self.extract_numeric_signals(
            announcement['title']
        )

        if has_numeric_signal:
            decision_log.append(
                f"✓ NUMERIC SIGNAL: {numeric_direction} (confidence: {numeric_confidence:.2f})"
            )
            # Override neutral sentiment if we have numeric signals
            if sentiment == 'neutral' and numeric_direction in ['POSITIVE', 'NEGATIVE']:
                sentiment = numeric_direction.lower()
                sentiment_score = numeric_confidence
                decision_log.append(
                    f"  → Overriding neutral sentiment with numeric signal: {sentiment}"
                )

        # Now check if we have a tradeable sentiment
        if sentiment not in ['positive', 'negative']:
            decision_log.append(f"❌ NEUTRAL SENTIMENT - No numeric signals detected")
            self._mark_processed(announcement['id'], filtered=True, reason="Neutral sentiment, no numeric signals")
            return None
        ...
        # 9. DECISION (REALISTIC MODE: Lower threshold to 0.4)
        if confidence < 0.4:
            decision_log.append(f"❌ CONFIDENCE TOO LOW ({confidence:.2f} < 0.4)")
